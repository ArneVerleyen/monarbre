import React from 'react';
import './about.scss';

const About = () => {
    return (
        <div className='about-container'>
            <h2>
                Wat doen we?
            </h2>
            <div className='card'>
                <h3>
                    We zorgen voor u bomen
                </h3>
                <p>
                    De nodige verzorging verschilt heel erg van boom tot boom, van omgeving tot omgeving, en wordt aangepast aan uw verwachtingen en de noden van uw bomen.
                </p>
            </div>
            <div className='card'>
                <h3>
                    GRATIS offerte
                </h3>
                <p>
                    We bezoeken uw bomen, om te kijken wat ze nodig hebben. U krijgt van ons gratis een voorstel van de optimale verzorging met bijhorende prijs.
                </p>
            </div>
            <div className='card'>
                <h3>
                    Advies in balans
                </h3>
                <p>
                    Na het bezoek aan uw bomen, raden we altijd de beste optie aan voor u en uw bomen.  We willen graag samen met u de verschillende mogelijkheden bespreken, om tot een goede balans te komen.
                </p>
            </div>
            <div className='card'>
                <h3>
                    Snoeien
                </h3>
                <p>
                    We snoeien alle bomen groot of klein.  We houden hier rekening met wanneer de boomsoort in kwestie het best gesnoeid kan worden en op welke manier we uw boom het best snoeien rekening houdend met de omstandigheden waarin uw boom verkeert.
                </p>
            </div>
            <div className='card'>
                <h3>
                    Vellen
                </h3>
                <p>
                    In eerste instantie bekijken we natuurlijk of de boom wel degelijk geveld moet worden en of er geen andere mogelijkheden zijn voor de boom in kwestie. 
                    Als de boom toch geveld moeten worden doen we dit op de meest veilige manier.
                    Alles wat in uw ogen niet mogelijk lijkt maken wij mogelijk. 
                </p>
            </div>
            <div className='card'>
                <h3>
                    Inspectie VTA
                </h3>
                <p>
                    We analyseren grondig de volledige boom en zijn omgeving. 
                    Dit doen we voor ieders veiligheid en uit respect voor de boom en zijn omgeving. 
                </p>
            </div>
            <div className='card'>
                <h3>
                    Standplaatsverbetering
                </h3>
                <p>
                    Sommige bomen staan niet ideaal waardoor ze het minder goed gaan doen. 
                    Wij analyseren wat we aan zijn staanplaats/situatie kunnen veranderen waardoor de boom meer kans en zin krijgt om beter te groeien en mooi in blad te staan.
                </p>
            </div>
            <div className='card'>
                <h3>
                    Boomstronken frezen
                </h3>
                <p>
                    Wanneer een boom geveld wordt blijft er altijd een stukje stam staan. Wij zorgen ervoor dat dit weg gefreest is en het lijkt alsof er nooit een boom gestaan heeft. 
                </p>
            </div>
            <div className='card'>
                <h3>
                    Aanplanting van bomen met nazorg
                </h3>
                <p>
                    We zorgen dat de bomen die we planten op de juiste manier en op een juiste plaats gepland worden. 
                    zo kunnen deze bomen optimaal groeien en oud worden. 
                    Geplante bomen snoeien we (indien nodig) op de juiste manier om het gewenste eindresultaat in de toekomst te krijgen.
                    We volgen ze goed op en zorgen dan ook dat de boom alles naar wens heeft.
                </p>
            </div>
            <div className='card'>
                <h3>
                    Inventaris maken
                </h3>
                <p>
                    Dit is aangeraden voor grotere domeinen. We maken een plan van het domein met de locatie van alle bomen. Elke boom wordt aangegeven met een nummer op het plan. Er worden van elke boom meerdere foto’s gemaakt, zodat we goed kunnen opvolgen hoe de boom evolueert. Alle werken die uitgevoerd worden aan een boom worden genoteerd in de inventaris. Deze inventarisatie laat toe om een goed overzicht te houden over de toestand en evolutie van alle bomen en zo de beste verzorging te kunnen garanderen.
                </p>
            </div>
        </div>
    );
};

export default About;